...
def _provenance_comment(comp, method):
    ...
    ``mirrored_from`` is recorded too, and it is the one field the EDITOR needs
    back: a link generated by ``mirror_limbs:`` has no entry in graph.json, so
    the URDF comment is the only place its provenance survives the build.
    Without it the editor shows a generated link as an ordinary one with no
    material and no CAD name, which reads like a broken extraction.
    """
    fields = []
    mirrored = getattr(comp, "mirrored_from", None)
    if mirrored:
        fields.append(f'mirrored_from="{_comment_safe(mirrored)}"')
