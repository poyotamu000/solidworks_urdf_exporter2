...
def _default_mass_links(pkg_dir, urdf_rel):
    ...
    # a link generated by `mirror_limbs:` has no component here at all, so it can
...
def _read_mirror_limbs(pkg_dir, urdf_rel):
    """The joints.yaml ``mirror_limbs:`` entries, or ``[]``.

    Only well-formed entries are returned: the client uses ``root`` to decide
    which link already generates a limb, and a malformed entry would make it
    offer to "undo" something it cannot find.
    """
    if not pkg_dir or not urdf_rel:
        return []
    name = os.path.splitext(os.path.basename(urdf_rel))[0]
    ...
    import yaml as _yaml
    try:
        specs = (_yaml.safe_load(open(yml, encoding="utf-8").read())
                 or {}).get("mirror_limbs") or []
    except Exception:
        return []
    return [s for s in specs if isinstance(s, dict) and s.get("root")]


def _link_mirror_sources(pkg_dir, urdf_rel):
    """``{link name -> the link it was mirrored from}`` from the built URDF.

    A link generated by ``mirror_limbs:`` exists only in the build -- there is no
    graph.json component behind it -- so every per-link lookup in this file
    (material, SolidWorks name, default-mass flag) comes back empty for it and it
    renders as a broken extraction.  The URDF's own provenance comment is where
    that fact survives; see urdf_writer._provenance_comment.  Empty on any parse
    failure.
    """
    if not pkg_dir or not urdf_rel:
        return {}
    ...
    for link in root.findall("link"):
        for node in link:
            if not callable(node.tag):          # comments have a callable tag
                continue
            m = re.search(r'mirrored_from="([^"]*)"', node.text or "")
            if m and link.get("name"):
                out[link.get("name")] = m.group(1)
    return out
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == "/api/components":
                ...
                mirror_sources = _link_mirror_sources(cls.pkg_dir, cls.urdf_rel)
                ...
                for dl in names:
                    ...
                    links[dl] = {
                        ...
                        # set when this LINK itself was generated by
                        # `mirror_limbs:` -- there is no CAD part behind it, so
                        # the material/name columns are empty by nature
                        "mirrored_from": mirror_sources.get(dl),
                ...
                return self._send_json({"links": links,
                                        ...
                                        # the `mirror_limbs:` entries, so a link
                                        # panel can show "this limb is mirrored"
                                        # and offer to undo it
                                        "mirror_limbs": _read_mirror_limbs(
            ...
            if path.startswith("/pkg/"):
                ...
                def _maybe_merge(txt):
                    ...
                    merge_fixed_links(
                        root, force_merge=_read_mass_only(cls.pkg_dir))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parsed.path == "/api/set_frame_only":
                ...
                return self._send_json({"link": link, "on": on})
            if parsed.path == "/api/set_mirror_limb":
                # Generate the limb on the other side of the robot from this one
                # (`mirror_limbs:`), or drop a previously generated one.  For an
                # assembly that only models torso + one arm + one leg.
                #
                # The build is the only thing that can say whether generation
                # actually worked -- a rename that collides, a plane that does
                # not fit, a limb whose meshes cannot be written all come back as
                # a refusal there.  So capture the build's own report and, if it
                # refused, ROLL THE CONFIG BACK and hand the reason to the user:
                # a green toast over a robot that did not change is exactly the
                # kind of silent success this editor should never show.
                cls = type(self)
                n = int(self.headers.get("Content-Length", 0))
                body = json.loads(self.rfile.read(n) or b"{}")
                link = body.get("link")
                ...
                if not _cad_mode(cls.pkg_dir):
                    return self._send_json(
                        {"error": "mirroring a limb needs a CAD package "
                                  "(graph.json)"}, 400)
                name = os.path.splitext(os.path.basename(cls.urdf_rel))[0]
                yml = os.path.join(cls.pkg_dir, name + ".joints.yaml")
                if not os.path.exists(yml):
                ...
                with open(yml, encoding="utf-8") as f:
                    txt = f.read()
                if re.search(r"(?m)^mirror_limbs:[ \t]*\S", txt):
                    return self._send_json(
                        {"error": "mirror_limbs: is in inline/flow style; "
                                  "reformat it to a block list first"}, 400)
                comp = _link_names_inverse(txt).get(link, link)
                specs = _read_mirror_limbs(cls.pkg_dir, cls.urdf_rel)
                idx = next((i for i, sp in enumerate(specs)
                            if sp.get("root") in (link, comp)), None)
                before = txt
                if on:
                    if idx is not None:
                        return self._send_json(
                    ...
                    if idx is None:
                        return self._send_json(
                            {"error": f"'{link}' does not mirror a limb"}, 400)
                    _snapshot(cls.pkg_dir, yml,
                              f"un-mirror limb {comp[:30]}")
                    txt = _remove_yaml_list_item(txt, "mirror_limbs", idx)
                with open(yml, "w", encoding="utf-8") as f:
                    f.write(txt)
                import contextlib
                import io as _io
                ...
                print(out, end="")
                refusal = re.search(
                    r"mirror_limbs '[^']*': NOT generated -- (.*)", out)
                if on and refusal:
                    with open(yml, "w", encoding="utf-8") as f:
                        f.write(before)
